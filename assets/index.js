...
function deleteOne(bool, stringa) {
  if (bool === true) {
...
function isThisAnEmail(stringa) {
  const email = stringa.split("@");
  if (stringa.includes("@") && email[1].includes(".")) {
    return true;
  }
}
...
const content = () => {
  const td = document.querySelectorAll("td");
  td.forEach((tdt) => console.log(tdt.textContent));
};
